...
public class StaffController {
    ...
    public boolean saveStaff(String firstName, String lastName, String nickname, String address, String zipCode, String postalAddress, String country, String email) {
        if (firstName.isEmpty() || lastName.isEmpty() || nickname.isEmpty() || address.isEmpty() || zipCode.isEmpty() || postalAddress.isEmpty() || country.isEmpty() || email.isEmpty()) {
            ...
            alert.setContentText("Please fill in all mandatory fields (All fields must be filled)!");
            alert.showAndWait();
    ...
    public void displayAddConfirmation() {
        ...
        confirmationAlert.setHeaderText("Add Staff Confirmation");
    ...
    public void displayUpdateFailure() {
        ...
        failureAlert.setHeaderText("Update Failure");
        ...
        emailField.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                ...
                if (!email.matches(emailRegex)) {
                    ...
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.getDialogPane().getStylesheets().add(Objects.requireNonNull(StaffController.class.getResource("/style.css")).toExternalForm());
                    alert.setTitle("Error");
                    alert.setHeaderText("Error");
    ...
    public boolean deleteStaff(int staffId) {
        boolean deleteSuccessful = delete(staffId);
        return deleteSuccessful;
    }

    public void searchStaff(TableView<Staff> staffTableView, TextField searchField, FilteredList<Staff> filteredStaff) {
        // Lyssnar på ändringar i sökfältet
        searchField.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredStaff.setPredicate(staff -> {
                // Om filtertexten är tom visas all personal
                if (newValue == null || newValue.isEmpty()) {
                    return true;
                }

                // Konvertera filtertexten till gemener
                String lowerCaseFilter = newValue.toLowerCase();

                // jämför med personalattribut (t.ex. förnamn, efternamn, osv.)
                return staff.getFirstName().toLowerCase().contains(lowerCaseFilter)
                        || staff.getLastName().toLowerCase().contains(lowerCaseFilter)
                        || staff.getNickname().toLowerCase().contains(lowerCaseFilter);
            });
        });

// filtrerade listan i en sorterad lista (SortedList)
        SortedList<Staff> sortedStaff = new SortedList<>(filteredStaff);

//Koppla TableView sorteringsfunktion till den sorterade listans sorteringsfunktion
        sortedStaff.comparatorProperty().bind(staffTableView.comparatorProperty());

// Sätt objekten i TableView
        staffTableView.setItems(sortedStaff);
    }
}